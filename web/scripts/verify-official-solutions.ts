...
const outputDir = path.join(root, "output", "puzzles");
...
const sourceFiles = (await fs.readdir(outputDir, { recursive: true }))
  .filter((name) => name.endsWith(".json"))
  .map((name) => path.join(outputDir, name));
const sources = await Promise.all(sourceFiles.map(async (file) => JSON.parse(await fs.readFile(file, "utf8")) as { target_solution_edges: string[]; metadata: { title: string } }));
const sourceById = new Map(sources.map((source) => [source.metadata.title, source]));
...
for (const item of index.puzzles) {
  ...
  if (!source) throw new Error(`${item.id} 找不到桌面版源题`);
  ...
  for (let i = 0; i < source.target_solution_edges.length; i += 1) {
    const shortened = source.target_solution_edges.filter((_, index) => index !== i);
