...
仓库同时包含移动优先的 React PWA。参与者扫描二维码后可在手机、平板或电脑上选择 20 道关卡，自动保存作答和计时，验证成功后生成截图凭证；网页题库不包含答案。

PWA 首页提供八步互动教程，通过可点击圆弧、错误分叉演示、单环判断，以及 π 统计范围、六类圆弧和“各一条”三段教学讲解规则；教程不计时，支持刷新续学和离线使用。
